...
class Session:
    ...
    jitter_pct
        Fraction of the current delay used as uniform jitter (0..jitter_pct
        of the delay, added on top).
    max_attempts
    ...
    on_event
        Optional observer invoked on every transition. Useful for hooking
        metrics, dashboards, or test assertions.
    """
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            log.warning("session.connect.failed", error=str(exc), error_type=type(exc).__name__)
    ...
    def _reconnect_loop(self) -> None:
        ...
        while True:
            ...
            if self.max_attempts and self._reconnect_attempts > self.max_attempts:
                log.error("session.reconnect.exhausted", attempts=self._reconnect_attempts)
            ...
            log.info(
                "session.reconnect.waiting",
                attempt=self._reconnect_attempts,
                delay_s=round(sleep_for, 3),
            )
    ...
    def check_health(self) -> bool:
        ...
        Transitions to DEGRADED if heartbeat is stale, or to RECONNECTING /
        DISCONNECTED if the underlying adapter dropped.
        """
        ...
        # Recover from DEGRADED if heartbeat came back fresh.

...
def _check_clock(config: AppConfig, skip: bool) -> dict[str, Any]:
    ...
    try:
        ...
        log.warning("bootstrap.clock_check.failed", error=str(exc), error_type=type(exc).__name__)
...
def main() -> int:
    ...
    try:
        ...
        log.error("bootstrap.connect.failed", error=str(exc), error_type=type(exc).__name__)
        manifest["stages"]["connectivity"] = {
            "status": "fail",
            "error": str(exc),
    ...
    try:
        ...
        log.error("bootstrap.resolve.failed", error=str(exc), error_type=type(exc).__name__)
        manifest["stages"]["contract_resolution"] = {
            "status": "fail",
            "error": str(exc),
            "error_type": type(exc).__name__,
    ...
    try:
        ...
        log.error("bootstrap.snapshot.failed", error=str(exc), error_type=type(exc).__name__)
        manifest["stages"]["snapshot"] = {
            "status": "fail",
            "error": str(exc),
            "error_type": type(exc).__name__,
...
def _write_manifest(manifest: dict[str, Any], config: AppConfig) -> None:
    ...
    log.info("bootstrap.manifest.written", path=str(path))

...
def configure_logging(
    ...
    # Bind base context once at startup. Subsequent calls extend, not replace.
    clear_contextvars()
    bind_contextvars(run_id=run_id)

    # Convert level string to numeric for stdlib root logger.
    numeric_level = getattr(logging, log_level.upper(), logging.INFO)

    # Stdlib logging is configured so that any third-party library logs (ib_async,
    # urllib3, etc.) are funneled through structlog and end up in the same files.
...
def _new_run_id() -> str:
    ...
    Sortable lexicographically by wall-clock start time, which is helpful
    when browsing log directories.
    """
